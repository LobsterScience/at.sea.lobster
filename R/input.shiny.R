...
fluidRow(
  ...
  fluidRow(
    column(2, selectInput("entry_group", "DATA OWNER GROUP", choices= c("","CBFH","CMM","DFO","ESFPA","GCIFA","MNCC","RCIFA","SWLSS"))),
...
####################################################################################################################################
  ...
  for (row_id in row_ids()) {
    ...
    data.row <- data.frame(
      ...
      length = as.character(round((input[[paste0("length_", row_id)]]),2)),
  ...
  update.set <- function(db=NULL, trip.id = NULL, set.id = NULL, delete = FALSE){
    ...
    if(nrow(set.result)==0){
      set.dat <- data.frame(
        ...
        est.catch = NA,
        input$num_traps,
        as.character(round((input$lat),3)),
        as.character(round((input$lon),3)),
        as.character(round((input$depth),2)),
        input$soak_days,
        source = NA,
        num.hooks = NA,
        input$trap_type,
